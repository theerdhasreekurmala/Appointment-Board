...
function renderAppointments(list) {
  ...
  board.innerHTML = '';
  if (!list.length) {
    board.innerHTML = '<p>No appointments found.</p>';
    return;
  }
  list.forEach((a, idx) => {
    ...
    card.innerHTML = `
      ...
      <p>${a.description || ''}</p>
      <div class="actions">
        <button data-id="${a.id}" class="edit btn-outlined"><span class="material-icons">edit</span></button>
        ${a.status === 'scheduled' ? `<button data-id="${a.id}" class="complete btn-outlined"><span class="material-icons">done</span></button>` : ''}
        ${a.status !== 'cancelled' ? `<button data-id="${a.id}" class="cancel btn-outlined"><span class="material-icons">cancel</span></button>` : ''}
      </div>
    `;
...
function showForm(mode='add', appt=null) {
  ...
  qs('#form-title').textContent = mode === 'add' ? 'Add Appointment' : 'Edit Appointment';
  ...
  if (btn) {
    ...
    if (btn.classList.contains('complete')) {
      ...
      if (!res.ok) {
        try {
          const err = await res.json();
          alert(err.error || 'Could not complete appointment');
        } catch (e) {
